...
(function () {
  ...
  function evalRubricHtml() {
    function group(title, arr) {
      return '<div class="aisum__eval-group"><h5>' + title + '</h5>'
    ...
    return '<details class="aisum__eval"><summary>📋 스낵 비장애 크루 평가 기준 <span class="muted">(참고)</span></summary>'
      + '<div class="aisum__eval-body">' + group("업무", EVAL_SNACK["업무"]) + group("역량", EVAL_SNACK["역량"]) + '</div></details>';
  }
  ...
  function crewSummaryCard(c) {
    ...
    if (crewSummaryOpen) {
      ...
      // 스낵 비장애 크루: 평가 기준(참고) 하단 접이식
      if (isManager && c.group === "스낵") body += evalRubricHtml();
